...
class Dynamic(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.W = nn.Parameter(torch.randn(136, 136, requires_grad = True, dtype = torch.float))
    ...
    def forward(self, inp, hidden):
        actual_inp = torch.matmul(inp.view(136, self.hidden_size), self.theta0) + \
                     torch.matmul(torch.matmul(self.W, inp.view(136, self.hidden_size)), self.theta1)
...
np.save('W_Relation_Matrix', W_final)
